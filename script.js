...
const config = {
    /**
     * Размер поля, массив скоростей движения змейки
     */
    size: 20,
    speed: [500, 350, 250, 150, 100]
};
...
const game = {
    ...
    */
    scoring: {
        ...
        scoreWrite: function () {
            let scoreNum = document.querySelector('#score-value');
            scoreNum.textContent = '';
            scoreNum.insertAdjacentHTML('beforeend', `<span id="score-value" class="value">${this.score}</span>`);
        }
    },
    ...
    start() {
        ...
        /*
        Проверяем текущий статус. Если он равен 'paused', просто меняем статус на 'started'
        Иначе (т.е.при старте игры) кроме смены статуса на 'started', отрисовываем элементы игры - поле, змейку и фрукты
        */
        let elt = document.getElementById('game');
        
        if (elt.classList[1] === 'paused') { 
    ...
    pause() {        
        ...
        interval = setInterval ( game.oneMove, 900000, SNAKE_DIRECTION_DOWN );    
               
    },

    /**
     * Функция останавливает игру. Очищает поле, меняет статус, выводит сообщение о завершении игры и накопленных очках
     */
    stop() {
    ...
    /**
     * Функция получает на вход направление движения, перемещает змейку, проверяет, съедена ли еда, изменяет счет
     *
     * @param direction Направление движения змейки.
     */   
    
     oneMove: function (direction) {
        ...
        if (foundFood !== -1) {
            /* устанавливаем следующую позицию змейки со вторым параметром "не удалять хвост змейки",
            ...
            game.scoring.scoreSet ();
            game.scoring.scoreWrite ();          
    ...
    /**
     * Функция выполняет самостоятельное передвижение змейки по полю. Вызывает функцию "oneMove"
     *
    ...
    move(event) {
         ...
         * устанавливаем соответствующее направление движения */        
        ...
        /*
        *  Устанавливаем скорость игры (с ростом набранных очков игра ускоряется)
        */
        switch (true) {

            case game.scoring.score < 5:
                setSpeed = config.speed [0];
                break;
            case game.scoring.score < 7:
                setSpeed = config.speed [1];
                break;
            case game.scoring.score < 15:
                setSpeed = config.speed [2];
                break;
            case game.scoring.score < 20:
                setSpeed = config.speed [3];
                break;
            default:
                setSpeed = config.speed [4];
        }

        clearInterval (interval);
        interval = setInterval ( game.oneMove, setSpeed, direction );
...
const board = {
    ...
    clear () {
        ...
        /* обнуляем змейку и еду, фиксируем счет */
        snake.parts = [];
        food.items = [];
        game.scoring.score = `Game Over with ${game.scoring.score} points`;

        /* перерендериваем змейку и еду, выводим на табло финальный счет и сообщение о завершении игры */
        snake.render();
        food.render ();        
        game.scoring.scoreWrite ();        
    }
...
const snake = {
    ...
    getNextPosition() {
        ...
        /* если змейка врезается в себя, заканчиваем игру по аналогии с наатием кнопки "Сдаться" */
        for (iter in snake.parts) {
            if (position.top === snake.parts[iter].top && position.left === snake.parts[iter].left) {
                game.stop ();
            }
        }
...
const food = {
    ...
    foundPosition(snakePosition) {
        /* здесь происходит вызов функции compareFunction для каждого элемента в массиве,
    ...
    generateItem() {
        ...
        /* создаем из копий массивов, содержащих координаты змейки и еды, объединенный массив
        *  проверяем координаты нового элемента на предмет попадания в объединенный массив
        * цель - не допустить, чтобы новая еда отрисовалась на змейке или совпала с координатами уже существующей еды  */
        let coordinatesToCheck = [...snake.parts];
        for (itr of [...this.items]) {
            coordinatesToCheck.push(itr)
        };
        ...
        while ( this.items.length < 3 ) {  
            ...
            for (iter of coordinatesToCheck) {
                if (newItem.top === iter.top && newItem.left === iter.left) {
                    const newItem = {
                    top: getRandomNumber(0, config.size - 1),
                    left: getRandomNumber(0, config.size - 1)
                    };
